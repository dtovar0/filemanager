...
def setup_web_server():
    ...
    if os.path.exists(nginx_tpl):
        ...
        with open('setup/filemanager-nginx.conf', 'w') as f:
            f.write(nginx_conf)
    else:
    ...
    if os.path.exists(apache_tpl):
        ...
        with open('setup/filemanager-apache.conf', 'w') as f:
            f.write(apache_conf)
    else:
    ...
    if os.path.exists(service_tpl):
        ...
        with open('setup/filemanager.service', 'w') as f:
    ...
    print(f"    - filemanager-apache.conf")
    print(f"    - filemanager-nginx.conf")
    print(f"    - filemanager.service")
    print(f"\n[!] Al finalizar, mueva los archivos a sus respectivas carpetas:")
    if server_choice == 'nginx':
        print(f"    sudo cp setup/filemanager-nginx.conf /etc/nginx/sites-available/filemanager")
        print(f"    sudo ln -s /etc/nginx/sites-available/filemanager /etc/nginx/sites-enabled/")
    else:
        print(f"    sudo cp setup/filemanager-apache.conf /etc/apache2/sites-available/filemanager.conf")
        print(f"    sudo a2ensite filemanager.conf && sudo a2enmod proxy proxy_http")
    
    print(f"    sudo cp setup/filemanager.service /etc/systemd/system/filemanager.service")
    print(f"    sudo systemctl daemon-reload")

def main():
    clear_screen()
    print("========================================")
    print("   FILEMANAGER - ASISTENTE DE INSTALACIÓN")
    print("========================================")
    ...
    try:
        ...
        db_name = input("Nombre de la base de datos [filemanager]: ") or "filemanager"
    ...
    admin_name = input("Nombre completo del admin [Admin FileManager]: ") or "Admin FileManager"
    admin_email = input("Correo electrónico del admin [admin@filemanager.local]: ") or "admin@filemanager.local"
    
    while True:
        admin_pass = getpass.getpass("Contraseña para el panel FileManager (Admin): ")
