...
async function refreshPicklistAreas(prefix, selectedNames = []) {
    let areas = window.__areaData || [];
    
    // Forzar recarga si parece vacío
    if (!areas || areas.length === 0) {
        try {
            const res = await fetch('/admin/areas-api');
            areas = await res.json();
            window.__areaData = areas;
        } catch (e) { console.error("API error during refresh:", e); }
    }
    
    const selected = Array.isArray(selectedNames) ? selectedNames : [];
    const available = (areas || []).filter(a => !selected.includes(a.name));
    const assigned = (areas || []).filter(a => selected.includes(a.name));
...
function populateUserAreasPicklist(containerId, items, prefix) {
    ...
    if (!items || items.length === 0) {
        if (!isSelected) {
            container.innerHTML = `<div class="text-center p-3 text-muted fs-xs">No hay áreas disponibles.</div>`;
        }
        return;
    }
    ...
    items.forEach(item => {
        ...
        } catch (err) { console.error("Error creating area card:", err); }
    });
}
...
function createAreaCardModern(area, isSelected, prefix) {
    ...
    const iconHtml = (area.icon && area.icon.startsWith('fa-')) 
        ? `<i class="fas ${area.icon}" style="font-size: 1.2rem; color: white;"></i>`
        : (iconsMap[area.icon] || iconsMap['box']);

    card.innerHTML = `
        <div class="card-icon" style="background: ${area.color || getAreaColor(area.name)}; display: flex; align-items: center; justify-content: center;">
            ${iconHtml}
        </div>
        <div class="card-info">
...
function updateActionButtons() {
    ...
    const btnPassword = document.getElementById('btnPasswordUser');

    if (btnEdit) btnEdit.disabled = (count !== 1);
    if (btnAccess) btnAccess.disabled = (count !== 1);
    if (btnPassword) btnPassword.disabled = (count !== 1);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', (event) => {
        ...
        if (action === 'users-open-password') return openChangePasswordModal();
        if (action === 'users-close-password-modal') return window.closeModal('changePasswordModal');
        if (action === 'users-submit-password') {
            event.preventDefault();
            return saveUserPassword(event);
        }
...
function changeUserStep(step) {
    ...
    if (step === 2) {
        ...
        // Forzar refresco al entrar al paso 2 por si acaso
...
async function saveUserChanges(e) {
    ...
    const nameEl = document.getElementById('editUserNameInput');
    const emailEl = document.getElementById('editUserEmailInput');
...
function openChangePasswordModal() {
    ...
    const idInput = document.getElementById('passwordUserId');
    const nameLabel = document.getElementById('passwordUserName');
    if (idInput) idInput.value = user.id;
    if (nameLabel) nameLabel.textContent = `Usuario: ${user.name}`;
...
async function saveUserPassword(e) {
    ...
    if (!pwd || pwd.length < 8) {
        return Toast.fire({ icon: 'warning', title: 'La contraseña debe tener al menos 8 caracteres' });
    }
    if (pwd !== confirm) {
    ...
    try {
        const res = await fetch(`/admin/change-password/${id}`, { 
            method: 'POST', 
            body: fd,
        ...
        if (data.success) {
            ...
            Toast.fire({ icon: 'success', title: 'Contraseña actualizada correctamente' });
        } else {
            Swal.fire('Error', data.error || 'No se pudo actualizar', 'error');
        }
    } catch (err) {
