...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Direct Sidebar Toggle Listener (More robust)
    ...
    // Global Picklist Filtering Logic
    document.addEventListener('input', (event) => {
        if (event.target.classList.contains('picklist-filter-input')) {
            const input = event.target;
            const term = input.value.toLowerCase();
            
            // Find the closest picklist pane to determine which list to filter
            const pane = input.closest('.picklist-pane');
            if (pane) {
                const list = pane.querySelector('.picklist-body');
                if (list) {
                    list.querySelectorAll('.picklist-card-premium').forEach(card => {
                        const name = card.querySelector('.card-name').textContent.toLowerCase();
                        const meta = card.querySelector('.card-meta').textContent.toLowerCase();
                        card.style.display = (name.includes(term) || meta.includes(term)) ? 'flex' : 'none';
                    });
                }
            }
        }
